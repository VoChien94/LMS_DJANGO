...
function TakeQuiz() {
  ...
  useEffect(() => {
    try {
      axios.get(`${BASE_URL}quiz-questions/${quiz_id}/1`)
  ...
  const submitAnswer = (question_id, right_ans) => {
    ...
    try {
      axios.post(`${BASE_URL}attempt-quiz/`, _formData, {
        headers: {
        ...
        .then((res) => {
          if (res.status == 200 || res.status == 201) {
            try {
             axios.get(`${BASE_URL}quiz-questions/${quiz_id}/next-question/${question_id}/`)
